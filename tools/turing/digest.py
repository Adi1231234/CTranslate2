...
once from the stock wheel (the reference never has to run again):
  enc_long / enc_mid / enc_6 / enc_1  every encoder output byte, batch sizes of both production paths
  ...
  beam_long_448                       the same with the end token suppressed: 448 steps, every
                                      self-attention width a transcription can reach
...
feats["6"], feats["1"] = feats["long"][:6], feats["long"][:1]
out, enc = {}, {}
...
run("beam_long_448", enc["long"], [prompt] * 8, forced=True, **batched)
...
summary = {"ctranslate2": ctranslate2.__file__, "seconds": round(time.time() - t0, 1),
